...
namespace BoingKit
{
  public class BoingBones : BoingReactor
  {
    ...
    public void RemoveBonesAfterTransformBone(Transform _transform)
    {
      var  boneIndex     = 0;
      ...
      {
        if (i < alreadyBoneList.Count && alreadyBoneList[i].Transform == _transform && !isRemoveBones)
        {
          boneIndex     = i;
          isRemoveBones = true;
        }
    ...
    // public void RemoveBonesAfterBoneIndex(int boneIndex)
    // {
    //   var loopCount = alreadyBoneList.Count;
    //   for (int i = 0; i < loopCount; i++)
    //   {
