        ...
        public void PullResults(BoingBones bones)
        {
          for (int iChain = 0; iChain < bones.BoneData.Length; ++iChain)
          {
            ...
            if (bones.EnableRotationEffect)
            {
              for (int iBone = 0; iBone < aBone.Length; ++iBone)
              {
                ...
                if (iBone == 0 && !chain.LooseRoot)
                {
                  ...
                  bone.BlendedRotationLs = bone.CachedRotationLs;
                  continue;
                }
                ...
                if (bone.ChildIndices == null)
                {
                  if (bone.ParentIndex >= 0)
                  {
                    ...
                    bone.BlendedRotationLs = parentBone.BlendedRotationLs *  bone.CachedRotationLs;
                  }
                ...
                if (totalWeight > 0.0f)
                {
                  ...
                  bone.BlendedRotationLs       = (boneRot * bone.RotationBackPropDeltaPs) * boneRot;
                }
            ...
            for (int iBone = 0; iBone < aBone.Length; ++iBone)
            {
              ...
              //Changed By Akash
              if (bone.tempFreezePositionRotation && bone.timer < bone.timeToFreeze)
              {
                bone.timer += Time.deltaTime;
                if (bone.timer >= bone.timeToFreeze)
                  bone.tempFreezePositionRotation = false;
              }
              
              if (bone.isFreezeRotationPosition || bone.tempFreezePositionRotation)
              {
                // Debug.Log(bone.Transform.name + "FreezeBone" );
                bone.Transform.localPosition = bone.Transform.localPosition;
              }
              else
              {
              ...
              if (bone.isFreezeRotationPosition || bone.tempFreezePositionRotation)
              {
                bone.Transform.localRotation = bone.Transform.localRotation;
              }
              else
              {
                bone.Transform.rotation = bone.isClampYRotation ? bone.BlendedRotationWs.ClampY(-.2f, .2f) : bone.BlendedRotationLs;
